...
/**
 * ArgParser is used to parse the command line arguments for a java application program. It provides a compact way to
 * specify options and match them against command line arguments, with support for <a href=#rangespec>range
 * checking</a>, <a href=#multipleOptionNames>multiple option names</a> (aliases), <a href=#singleWordOptions>single
 * word options</a>, <a href=#multipleOptionValues>multiple values associated with an option</a>, <a
 * href=#multipleOptionInvocation>multiple option invocation</a>, <a href=#helpInfo>generating help information</a>, <a
 * href=#customArgParsing>custom argument parsing</a>, and <a href=#argsFromAFile>reading arguments from a file</a>. The
 * last feature is particularly useful and makes it easy to create ad-hoc configuration files for an application.
 * 
 ...
 *   parser.addOption("-theta %f #theta value (in degrees)",
 *                    theta);
 *   parser.addOption("-file %s #name of the operating file",
 *                    fileName);
 ...
 * {@link argparser.StringHolder StringHolder}), an array of the appropriate type, or <a href=#multipleOptionInvocation>
 * an instance of <code>java.util.Vector</code></a>.
 * 
 * <p>
 ...
 *
 * parser.addOption("-name %s {john,mary,jane}",
 *                  name);
 * parser.addOption("-index %d {[1,256]}",
 *                  index);
 * parser.addOption("-size %d {1,2,4,8,16}",
 *                  size);
 * parser.addOption("-foo %f {(-99,-50],[50,99)}",
 *                  foo);
 * </pre>
 * 
 ...
 *
 * addOption("-position %fX3 #position of the object",
 *           pos);
 * </pre>
 * 
 ...
 * {@link #ArgParser(String,boolean) ArgParser(synopsis,false)}. Help options can also be specified by the application
 * using {@link #addOption addOption} and the conversion code <code>%h</code>. Help options can be disabled using
 * {@link #setHelpOptionsEnabled setHelpOptionsEnabled(false)}.
 * 
 * <p>
 ...
 * 
 * <pre>
 * parser.addOption("-theta %f #NUMBER#theta value (in degrees)",
 *                  theta);
 * </pre>
 * 
 * instead of
 ...
 * 
 * <p>
 * First, the method {@link #matchAllArgs(String[],int,int) matchAllArgs(args,idx,exitFlags)} returns an array of all
 * unmatched arguments, which can then be handled specially:
 * 
 * <pre>
 *    String[] unmatched =
...
{
  Vector<Record> m_matchList;

  // int tabSpacing = 8;
  String m_synopsisString;
  boolean m_helpOptionsEnabled = true;
  Record m_defaultHelpOption = null;
  Record m_firstHelpOption = null;
  PrintStream m_printStream = System.out;
  int m_helpIndent = 24;
  String m_errMsg = null;
  String m_unmatchedArg = null;
  static String m_validConversionCodes = "iodxcbfsvh";

  /**
   * Indicates that the program should exit with an appropriate message in the event of an erroneous or malformed
   * argument.
   */
   ...
   */
  public static final int EXIT_ON_UNMATCHED = 2;

  /**
   * Returns a string containing the valid conversion codes. These are the characters which may follow the
   * <code>%</code> character in the specification string of {@link #addOption addOption}.
   * 
  ...
  static class RangePnt
  {
    ...
    long mi_lval = 0;
    String mi_sval = null;
    boolean mi_bval = true;
    boolean mi_closed = true;
    ...
    RangePnt(StringScanner scanner,
        int type)
        throws IllegalArgumentException
    {
      ...
      {
        throw new IllegalArgumentException("Malformed " + typeName + " '" + scanner.substring(scanner.getIndex(),
                                                                                              e.getFailIndex() + 1)
            + "' in range spec");
      }
    ...
    {
      return Double.compare(mi_dval, d);
    }
    ...
    {
      return "{ dval=" + mi_dval + ", lval=" + mi_lval + ", sval=" + mi_sval + ", bval=" + mi_bval + ", closed="
          + mi_closed + "}";
    }
  }
  ...
  class RangeAtom
  {
    RangePnt mi_low = null;
    RangePnt mi_high = null;
    RangeAtom mi_next = null;
        ...
        throws IllegalArgumentException
    {
      ...
      switch (type)
      {
        ...
        case Record.DOUBLE:
        {
          cmp = p0.compareTo(p1.mi_dval);
          break;
        }

        case Record.STRING:
        {
          cmp = p0.compareTo(p1.mi_sval);
          break;
        }
        default:
          assert false : "Unknown type " + type;
    ...
    {
      int lc = mi_low.compareTo(d);
      if (mi_high != null)
      {
        int hc = mi_high.compareTo(d);
        return (lc * hc < 0 || (mi_low.mi_closed && lc == 0) || (mi_high.mi_closed && hc == 0));
      }
      else
      {
        return lc == 0;
    ...
    {
      int lc = mi_low.compareTo(l);
      if (mi_high != null)
      {
        int hc = mi_high.compareTo(l);
        return (lc * hc < 0 || (mi_low.mi_closed && lc == 0) || (mi_high.mi_closed && hc == 0));
      }
      else
      {
    ...
    {
      int lc = mi_low.compareTo(s);
      if (mi_high != null)
      {
        int hc = mi_high.compareTo(s);
        return (lc * hc < 0 || (mi_low.mi_closed && lc == 0) || (mi_high.mi_closed && hc == 0));
      }
      else
      {
    ...
    {
      return mi_low.compareTo(b) == 0;
    }

    @Override
    public String toString()
    {
      return "low=" + (mi_low == null ? "null" : mi_low.toString()) + ", high="
          + (mi_high == null ? "null" : mi_high.toString());
    }
  }

  class Record
  {
    NameDesc mi_nameList;
    ...
    int mi_type;
    int mi_numValues;
    boolean mi_mandatory;
    String mi_helpMsg = null;
    String mi_valueDesc = null;
    String mi_rangeDesc = null;
    Object mi_resHolder = null;
    RangeAtom mi_rangeList = null;
    RangeAtom mi_rangeTail = null;
    char mi_convertCode;
    boolean mi_vval = true; // default value for now
    ...
    {
      int cnt = 0;
      for (RangeAtom ra = mi_rangeList; ra != null; ra = ra.mi_next)
      {
        cnt++;
      }
      return cnt;
    ...
    {
      if (mi_rangeList == null)
      {
        mi_rangeList = ra;
      }
      else
      {
        mi_rangeTail.mi_next = ra;
      }
      mi_rangeTail = ra;
    }
        ...
        throws ArgParseException
    {
      ...
      try
      {
        switch (mi_convertCode)
        {
          ...
          default:
            assert false : "Unknown char " + mi_convertCode;
        }
      }
      catch (StringScanException e)
      {
        throw new ArgParseException(name,
                                    "malformed " + valTypeName() + " '" + s + "'");
      }
      ...
      switch (mi_type)
      {
        ...
        default:
          assert false : "Unknown type " + mi_type;
      }
      if (outOfRange)
      {
        throw new ArgParseException(name,
                                    "value '" + s + "' not in range " + mi_rangeDesc);
      }
      ...
      else
      {
        switch (mi_type)
        {
          ...
          case BOOLEAN:
          {
            ((BooleanHolder) result).setValue(bval);
            break;
          }

          case CHAR:
          {
            ((CharHolder) result).setValue((char) lval);
            break;
          }

          case INT:
          {
            ((IntHolder) result).setValue((int) lval);
            break;
          }
          ...
          case FLOAT:
          {
            ((FloatHolder) result).setValue((float) dval);
            break;
          }

          case DOUBLE:
          {
            ((DoubleHolder) result).setValue(dval);
            break;
          }
  ...
  {
    if (m_firstHelpOption != null && m_firstHelpOption.mi_nameList != null)
    {
      return m_firstHelpOption.mi_nameList.mi_name;
    }
    else
    {
      return null;
  ...
  /**
   * Creates an <code>ArgParser</code> with a synopsis string, and the default help options <code>-help</code> and
   * <code>-&#063;</code>.
   * 
   * @param synopsisString
   *          string that briefly describes program usage, for use by {@link #getHelpMessage getHelpMessage}.
  ...
  /**
   * Creates an <code>ArgParser</code> with a synopsis string. The help options <code>-help</code> and <code>-?</code>
   * are added if <code>defaultHelp</code> is true.
   * 
   * @param synopsisString
   *          string that briefly describes program usage, for use by {@link #getHelpMessage getHelpMessage}.
   * @param defaultHelp
  ...
  public ArgParser(String synopsisString,
      boolean defaultHelp)
  {
    m_matchList = new Vector<Record>(128);
    m_synopsisString = synopsisString;
    if (defaultHelp)
    {
      addOption("-help,-? %h #displays help information",
                null);
      m_defaultHelpOption = m_firstHelpOption = m_matchList.get(0);
    }
  }

  /**
   * Returns the synopsis string used by the parser. The synopsis string is a short description of how to invoke the
   * program, and usually looks something like
   * <p>
  ...
  {
    return m_helpOptionsEnabled;
  }

  /**
   * Enables or disables help options. Help options are those associated with a conversion code of <code>%h</code>. If
   * help options are enabled, and a help option is matched, then the string produced by {@link #getHelpMessage
   * getHelpMessage} is printed to the default print stream and the program exits with code 0. Otherwise, arguments
   * which match help options are ignored.
   * 
   * @param enable
  ...
  /**
   * Sets the indentation used by {@link #getHelpMessage getHelpMessage}. This is the number of columns that an option's
   * help information is indented. If the option's name and value information can fit within this number of columns,
   * then all information about the option is placed on one line. Otherwise, the indented help information is placed on
   * a separate line.
   * 
   * @param indent
  ...
  // { return tabSpacing;
  // }
  private void scanRangeSpec(Record rec,
      String s)
      throws IllegalArgumentException
  {
    StringScanner scanner = new StringScanner(s);
    ...
    if (c != '{')
    {
      throw new IllegalArgumentException("Start of range '{' expected");
    }
    ...
    {
      rec.mi_rangeDesc = s.substring(1,
                                     s.length() - 1);
    }
    else
    {
  ...
  /**
   * Adds a new option description to the parser. The method takes two arguments: a specification string, and a result
   * holder in which to store the associated value.
   * 
   * <p>
   ...
   * <tr>
   * <td><code>%i</code></td>
   * <td>an integer, in either decimal, hex (if preceeded by <code>0x</code>), or octal (if preceeded by <code>0</code>)
   * </td>
   * <tr valign=top>
   * <td><code>%d</code></td>
   ...
   * <code>"foo"</code> or <code>"bar"</code>, plus any string lexically greater than or equal to <code>"aaa"</code> but
   * less then <code>"zzz"</code>.
   * 
   * <p>
   * <li><var>multiplier</var> is an optional integer, following a <code>X</code> character, indicating the number of
   * values which the option expects. If the multiplier is not specified, it is assumed to be 1. If the multiplier value
   * is greater than 1, then the result holder should be either an array (of appropriate type) with a length greater
   * than or equal to the multiplier value, or a <code>java.util.Vector</code> <a href=#vectorHolder>as discussed
   * below</a>.
   * 
   * <p>
   * <li><var>valueDescription</var> is an optional description of the option's value requirements, and consists of all
   * characters between two <code>#</code> characters. The final <code>#</code> character initiates the <i>option
   ...
   * 
   * <p>
   * The result holder must be an object capable of holding a value compatible with the conversion code, or it must be a
   * <code>java.util.Vector</code>. When the option is matched, its associated value is placed in the result holder. If
   * the same option is matched repeatedly, the result holder value will be overwritten, unless the result holder is a
   * <code>java.util.Vector</code>, in which case new holder objects for each match will be allocated and added to the
   * vector. Thus if multiple instances of an option are desired by the program, the result holder should be a
   ...
   * result holder object and add it to the vector. Multiple occurances of the option will cause multiple results to be
   * added to the vector.
   * 
   * <p>
   ...
   * @param resHolder
   *          object in which to store the associated value
   * @throws IllegalArgumentException
   *           if there is an error in the specification or if the result holder is of an invalid type.
   */
  public final void addOption(String spec,
      Object resHolder)
      throws IllegalArgumentException
  {
    addOption(spec,
      ...
      throws IllegalArgumentException
  {
    ...
    do
    {
      ...
      scanner.skipWhiteSpace();
      i0 = scanner.getIndex();
      while (!Character.isWhitespace(c = scanner.getc()) && c != ',' && c != '%' && c != '\000')
      {
        ;
      }
      i1 = scanner.getIndex();
      ...
      ndesc.mi_name = scanner.substring(i0,
                                        i1);
      if (rec == null)
      {
        rec = new Record();
        rec.mi_nameList = ndesc;
      }
      else
      {
        nameTail.mi_next = ndesc;
      }
      nameTail = ndesc;
      ndesc.mi_oneWord = !nameEndsInWhiteSpace;
    }
    while (c != '%');

    if (!nameTail.mi_oneWord)
    {
      for (ndesc = rec.mi_nameList; ndesc != null; ndesc = ndesc.mi_next)
      {
        ndesc.mi_oneWord = false;
      }
    }
    c = scanner.getc();
    if (c == '\000')
    ...
    if (m_validConversionCodes.indexOf(c) == -1)
    {
      throw new IllegalArgumentException("Conversion code '" + c + "' not one of '" + m_validConversionCodes + "'");
    }
    rec.mi_convertCode = c;
    rec.mi_mandatory = mandatory;

    if (resHolder instanceof Vector)
    {
      rec.mi_type = defaultResultType(rec.mi_convertCode);
    }
    else
    {
      switch (rec.mi_convertCode)
      {
        ...
        case 'x':
        {
          ...
          {
            rec.mi_type = Record.LONG;
          }
          else if (resHolder instanceof IntHolder || resHolder instanceof int[])
          {
            rec.mi_type = Record.INT;
          }
          else
          {
        ...
        case 'f':
        {
          ...
          {
            rec.mi_type = Record.DOUBLE;
          }
          else if (resHolder instanceof FloatHolder || resHolder instanceof float[])
          {
            rec.mi_type = Record.FLOAT;
          }
          else
          {
        ...
        case 'h':
          break;
        default:
          assert false : "Unknown convert code " + rec.mi_convertCode;
      }
    }
    if (rec.mi_convertCode == 'h')
    {
      rec.mi_resHolder = null;
    }
    else
    {
      rec.mi_resHolder = resHolder;
    }
    ...
    if (scanner.peekc() == '{')
    {
      ...
      scanRangeSpec(rec,
                    scanner.substring(i0,
                                      i1));
      if (rec.mi_convertCode == 'v' && rec.mi_rangeList != null)
      {
        rec.mi_vval = rec.mi_rangeList.mi_low.mi_bval;
      }
    }
    ...
    if (rec.mi_numValues > 1)
    {
      for (ndesc = rec.mi_nameList; ndesc != null; ndesc = ndesc.mi_next)
      {
        if (ndesc.mi_oneWord)
        {
          throw new IllegalArgumentException("Multiplier value incompatible with one word option " + ndesc.mi_name);
        }
      }
    }
    if (resHolder != null && resHolder.getClass().isArray())
    {
      if (Array.getLength(resHolder) < rec.mi_numValues)
      {
        throw new IllegalArgumentException("Result holder array must have a length >= " + rec.mi_numValues);
      }
    }
    else
    {
      if (rec.mi_numValues > 1 && !(resHolder instanceof Vector))
      {
        throw new IllegalArgumentException("Multiplier requires result holder to be an array of length >= "
            + rec.mi_numValues);
      }
    }
    ...
    if (!scanner.atEnd())
    {
      ...
      if (k != -1)
      {
        rec.mi_valueDesc = helpInfo.substring(0,
                                              k);
        rec.mi_helpMsg = helpInfo.substring(k + 1);
      }
      else
      {
        rec.mi_helpMsg = helpInfo;
      }
    }
    else
    {
    ...
    if (rec.mi_convertCode == 'h' && m_firstHelpOption == m_defaultHelpOption)
    {
      m_matchList.remove(m_defaultHelpOption);
      m_firstHelpOption = rec;
    }
    m_matchList.add(rec);
  }
  ...
  {
    return m_matchList.lastElement();
  }

  private Record getRecord(String arg,
      ObjectHolder ndescHolder)
  {
    NameDesc ndesc;
    for (int i = 0; i < m_matchList.size(); i++)
    {
      Record rec = m_matchList.get(i);
      for (ndesc = rec.mi_nameList; ndesc != null; ndesc = ndesc.mi_next)
      {
        if (rec.mi_convertCode != 'v' && ndesc.mi_oneWord)
        {
          if (arg.startsWith(ndesc.mi_name))
          {
            if (ndescHolder != null)
            {
  ...
  {
    Record rec = getRecord(arg,
                           null);
    return (rec != null) ? rec.mi_resHolder : null;
  }
  ...
  String getOptionName(String arg)
  {
    ...
    Record rec = getRecord(arg,
                           ndescHolder);
    return (rec != null) ? ((NameDesc) ndescHolder.getValue()).mi_name : null;
  }
  ...
  String getOptionRangeDesc(String arg)
  {
    Record rec = getRecord(arg,
                           null);
    return (rec != null) ? rec.mi_rangeDesc : null;
  }
  ...
  private Object createResultHolder(Record rec)
  {
    if (rec.mi_numValues == 1)
    {
      switch (rec.mi_type)
      {
        ...
        default:
          assert false : "Unknown type " + rec.mi_type;
      }
    }
    else
    {
      switch (rec.mi_type)
      {
        ...
        case Record.LONG:
          return new long[rec.mi_numValues];

        case Record.CHAR:
          return new char[rec.mi_numValues];

        case Record.BOOLEAN:
          return new boolean[rec.mi_numValues];

        case Record.DOUBLE:
          return new double[rec.mi_numValues];

        case Record.STRING:
          return new String[rec.mi_numValues];
        default:
          assert false : "Unknown type " + rec.mi_type;
      }
    }
  ...
  static void stringToArgs(Vector<String> vec,
      String s,
  ...
  /**
   * Reads in a set of strings from a reader and prepends them to an argument list. Strings are delimited by either
   * whitespace or double quotes <code>"</code>. The character <code>#</code> acts as a comment character, causing input
   * to the end of the current line to be ignored.
   * 
   * @param reader
   *          Reader from which to read the strings
   * @param args
   *          Initial set of argument values. Can be specified as <code>null</code>.
   * @throws IOException
  ...
  /**
   * Reads in a set of strings from a file and prepends them to an argument list. Strings are delimited by either
   * whitespace or double quotes <code>"</code>. The character <code>#</code> acts as a comment character, causing input
   * to the end of the current line to be ignored.
   * 
   * @param file
      ...
      throws IOException
  {
    ...
    try
    {
      return prependArgs(new FileReader(file),
                         args);
    }
    catch (IOException e)
    {
  ...
  /**
   * Prints an error message, along with a pointer to help options, if available, and causes the program to exit with
   * code 1.
   */
  public void printErrorAndExit(String msg)
  {
    if (m_helpOptionsEnabled && firstHelpOptionName() != null)
    {
      msg += "\nUse " + firstHelpOptionName() + " for help information";
    }
    if (m_printStream != null)
    {
      m_printStream.println(msg);
    }
    System.exit(1);
  }
   ...
   * {@link #getHelpMessage getHelpMessage} is printed to the default print stream and the program exits with code 0. If
  ...
  {
    return matchAllArgs(args,
                        0,
                        EXIT_ON_UNMATCHED | EXIT_ON_ERROR);
  }

  /**
   * Matches arguments within an argument list and returns those which were not matched. The matching starts at a
   * location in <code>args</code> specified by <code>idx</code>, and unmatched arguments are returned in a String
   * array.
   * 
   * <p>
   ...
   * 
   * <p>
   * In the event of an umatched argument, the method will print a message and exit if {@link #EXIT_ON_UNMATCHED} is set
   * in <code>errorFlags</code>. Otherwise, the unmatched argument will be appended to the returned array of unmatched
   * values, and the matching will continue at the next location.
   * 
   * <p>
   * If help options are enabled and one of the arguments matches a help option, then the result of
   * {@link #getHelpMessage getHelpMessage} is printed to the the default print stream and the program exits with code
   ...
   * @param exitFlags
   *          conditions causing the program to exit. Should be an or-ed combintion of {@link #EXIT_ON_ERROR} or
  ...
  public String[] matchAllArgs(String[] args,
      ...
      int exitFlags)
  {
    List<String> unmatched = new ArrayList<String>(10);
    ...
    while (idx < args.length)
    {
      try
      {
        idx = matchArg(args,
                       idx);
        if (m_unmatchedArg != null)
        {
          if ((exitFlags & EXIT_ON_UNMATCHED) != 0)
          {
            printErrorAndExit("Unrecognized argument: " + m_unmatchedArg);
          }
          else
          {
            unmatched.add(m_unmatchedArg);
          }
        }
      }
    ...
    for (Record r : m_matchList)
    {
      if (r.mi_mandatory && r.mi_resHolder != null)
      {
        if (r.mi_resHolder instanceof Holder && !((Holder) r.mi_resHolder).isInitialized())
        {
          printErrorAndExit("Missing argument: " + getOptionInfo(r));
        }

        if (r.mi_resHolder instanceof Vector && ((Vector) r.mi_resHolder).isEmpty())
        {
          printErrorAndExit("Missing argument: " + getOptionInfo(r));
        }
      }
    ...
    {
      return unmatched.toArray(new String[unmatched.size()]);
    }
  }

  /**
   * Matches one option starting at a specified location in an argument list. The method returns the location in the
   * list where the next match should begin.
   * 
   * <p>
   ...
   * @throws ArgParseException
   *           if there was an error performing the match (such as improper or insufficient values).
   * @see ArgParser#setDefaultPrintStream
   * @see ArgParser#getHelpOptionsEnabled
   * @see ArgParser#getErrorMessage
      ...
      throws ArgParseException
  {
    ...
    {
      ObjectHolder ndescHolder = new ObjectHolder();
      Record rec = getRecord(args[idx],
                             ndescHolder);
      if (rec == null || (rec.mi_convertCode == 'h' && !m_helpOptionsEnabled))
      { // didn't match
        m_unmatchedArg = args[idx];
        return idx + 1;
      }
      NameDesc ndesc = (NameDesc) ndescHolder.getValue();
      Object result;
      if (rec.mi_resHolder instanceof Vector)
      {
        result = createResultHolder(rec);
      }
      else
      {
        result = rec.mi_resHolder;
      }
      if (rec.mi_convertCode == 'h')
      {
        if (m_helpOptionsEnabled)
        {
          m_printStream.println(getHelpMessage());
          System.exit(0);
        }
        else
        {
      ...
      else if (rec.mi_convertCode != 'v')
      {
        if (ndesc.mi_oneWord)
        {
          assert ndesc.mi_name != null : "Expecting a non-null name here";
          rec.scanValue(result,
                        ndesc.mi_name,
                        args[idx].substring(ndesc.mi_name.length()),
                        0);
        }
        else
        {
          if (idx + rec.mi_numValues >= args.length)
          {
            throw new ArgParseException(ndesc.mi_name,
                                        "requires " + rec.mi_numValues + " value" + (rec.mi_numValues > 1 ? "s" : ""));
          }
          for (int k = 0; k < rec.mi_numValues; k++)
          {
            rec.scanValue(result,
                          ndesc.mi_name,
                          args[++idx],
      ...
      {
        if (rec.mi_resHolder instanceof BooleanHolder)
        {
          ((BooleanHolder) result).setValue(rec.mi_vval);
        }
        else
        {
          for (int k = 0; k < rec.mi_numValues; k++)
          {
            ((boolean[]) result)[k] = rec.mi_vval;
          }
        }
      }
      if (rec.mi_resHolder instanceof Vector)
      {
        ((Vector) rec.mi_resHolder).add(result);
      }
    }
    catch (ArgParseException e)
    {
  ...
  {
    StringBuilder sbuf = new StringBuilder(n);
    for (int i = 0; i < n; i++)
    {
      sbuf.append(' ');
    }
  ...
  public String getHelpMessage()
  {
    ...
    s = "Usage: " + m_synopsisString + "\n";
    s += "Options include:\n\n";
    for (int i = 0; i < m_matchList.size(); i++)
    {
      rec = m_matchList.get(i);
      if (rec.mi_convertCode == 'h' && !m_helpOptionsEnabled)
      {
        continue;
      }

      String optionInfo = getOptionInfo(rec);
      s += optionInfo;
      if (rec.mi_helpMsg.length() > 0)
      {
        int pad = m_helpIndent - optionInfo.length();
        if (pad < 2)
        {
          s += '\n';
          pad = m_helpIndent;
        }
        s += spaceString(pad) + rec.mi_helpMsg;
      }
  ...
  private String getOptionInfo(Record rec)
  {
    NameDesc ndesc;
    ...
    for (ndesc = rec.mi_nameList; ndesc != null; ndesc = ndesc.mi_next)
    {
      optionInfo += ndesc.mi_name;
      if (hasOneWordAlias && !ndesc.mi_oneWord)
      {
        optionInfo += " ";
      }
      if (ndesc.mi_next != null)
      {
        optionInfo += ",";
      }
    }
    ...
    if (rec.mi_convertCode != 'v' && rec.mi_convertCode != 'h')
    {
      if (rec.mi_valueDesc != null)
      {
        optionInfo += rec.mi_valueDesc;
      }
      else
      {
        if (rec.mi_rangeDesc != null)
        {
          optionInfo += "<" + rec.valTypeName() + " " + rec.mi_rangeDesc + ">";
        }
        else
        {
  ...
  /**
   * Returns the parser's error message. This is automatically set whenever an error is encountered in
   * <code>matchArg</code> or <code>matchAllArgs</code>, and is automatically set to <code>null</code> at the beginning
   * of these methods.
   * 
   ...
   * {@link #matchAllArgs(String[],int,int) matchAllArgs}. If there was no unmatched argument, <code>null</code> is
   * returned.
   * 
