...
public class Diff
{
   ...
   * With this speed_large_files, for files with a constant small density of
   ...
   * This makes things slower, sometimes much slower.
   */
  ...
  /*
   * Vector, indexed by diagonal, containing the X coordinate of the point
   * furthest along the given diagonal in the backward search of the edit matrix.
   */
   ...
   * translated to an "equivalence number" based on the result of
  ...
  /**
   * Find the midpoint of the shortest edit script for a specified portion of the
   * two files. We scan from the beginnings of the files, and simultaneously from
   * the ends, doing a breadth-first search through the space of edit-sequence.
   * When the two searches meet, we have found the midpoint of the shortest edit
   * sequence. The value returned is the number of the diagonal on which the
   * midpoint lies. The diagonal number equals the number of inserted lines minus
   * the number of deleted lines (counting only lines before the midpoint). The
   * edit cost is stored into COST; this is the total number of lines inserted or
   * deleted (counting only lines before the midpoint). This function assumes that
   * the first lines of the specified portions of the two files do not match, and
   * likewise that the last lines do not match. The caller must trim matching
   * lines from the beginning and end of the portions it is going to specify. Note
   * that if we return the "wrong" diagonal value, or if the value of bdiag at
   * that diagonal is "wrong", the worst this can do is cause suboptimal diff
  ...
  private void diag(int xoff, int xlim, int yoff, int ylim, boolean find_minimal, Partition part)
  {
    ...
    for (int c = 1;; ++c)
    {
       ...
       * speed_large_files, for files with a constant small density of changes, the
      ...
      /*
       * Heuristic: if we've gone well beyond the call of duty, give up and report
       * halfway between our best results so far.
       */
      if (c >= too_expensive)
      {
        int fxybest;
   ...
   * as a whole, to match each other. The results are recorded in the vectors
   * filevec[N].changed_flag, by storing a 1 in the element for each line that is
   * an insertion or deletion. The subsequence of file 0 is [XOFF, XLIM) and
   * likewise for file 1. Note that XLIM, YLIM are exclusive bounds. All line
   * numbers are origin-0 and discarded lines are not counted.
   */
  ...
  public change diff_2()
  {
    ...
    compareseq(0, filevec[0].nondiscarded_lines, 0, filevec[1].nondiscarded_lines, minimal);
    fdiag = null;
    bdiag = null;

    /*
     * Modify the results slightly to make them prettier in cases where that can
     * validly be done.
     */
    shift_boundaries();

    /*
     * Get the results of comparison in the form of a chain of `struct change's --
     * an edit script.
     */
    return build_script();
  }
  ...
  /**
   * The result of comparison is an "edit script": a chain of change objects. Each
   * change represents one place where some lines are deleted and some are
   * inserted. LINE0 and LINE1 are the first affected lines in the two files
   * (origin 0). DELETED is the number of lines deleted here from file 0. INSERTED
   * is the number of lines inserted here in file 1. If DELETED is 0 then LINE0 is
   * the number of the line before which the insertion was done; vice versa for
   * INSERTED and LINE1.
   */
  public static class change
  {
     ...
     * LINE1 are the first affected lines in the two files (origin 0). DELETED is
     * the number of lines deleted here from file 0. INSERTED is the number of lines
     * inserted here in file 1. If DELETED is 0 then LINE0 is the number of the line
     * before which the insertion was done; vice versa for INSERTED and LINE1.
     */
  ...
  class file_data
  {
     ...
     * will not be considered by the actual comparison algorithm; it will be as if
     * that line were not in the file. The file's `realindexes' table maps virtual
     * line numbers (which don't count the discarded lines) into real line numbers;
     * this is how the actual comparison algorithm produces results that are
      ...
      for (int i = 0; i < end; i++)
      {
        ...
        else if (discards[i] != 0)
        {
          ...
          else
          {
             ...
             * provisionals can stand when LENGTH is at least 16. A subrun of 4 or more can
             ...
             * or until the first nonprovisional at least 8 lines in. Until that point,
    ...
    /**
     * Adjust inserts/deletes of blank lines to join changes as much as possible. We
     * do something when a run of changed lines include a blank line at one end and
     * have an excluded blank line at the other. We are free to choose which blank
     * line is included. `compareseq' always chooses the one at the beginning, but
     * usually it is cleaner to consider the following blank line to be the
     * "change". The only exception is if the preceding blank line would join this
     * change to other changes.
     * 
    ...
    void shift_boundaries(file_data f)
    {
      ...
      for (;;)
      {
        ...
        /*
         * Scan forwards to find beginning of another run of changes. Also keep track of
         * the corresponding point in the other file.
         */
        while (i < i_end && !changed[1 + i])
        {
          ...
          {
            /*
             * Non-corresponding lines in the other file will count as the preceding batch
             * of changes.
             */
            other_preceding = j;
          }
        ...
        for (;;)
        {
              ...
              && !((preceding >= 0 && start == preceding) || (other_preceding >= 0 && other_start == other_preceding)))
          {
            ...
            ++i;
            /*
             * Since one line-that-matches is now before this run instead of after, we must
             * advance in the other file to keep in synch.
             */
            ++j;
          }
          else
     ...
     * It is this vector that is actually compared with that of another file to
     * generate differences.
     */
    private final int[] equivs;

    /**
     * Vector, like the previous one except that the elements for discarded lines
     * have been squeezed out.
     */
    final int[] undiscarded;

    /**
     * Vector mapping virtual line numbers (not counting discarded lines) to real
     * ones (counting those lines). Both are origin-0.
     */
    ...
    /**
     * Array, indexed by real origin-1 line number, containing true for a line that
     * is an insertion or a deletion. The results of comparison are stored here.
     */
