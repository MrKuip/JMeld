 ...
 * <p>
 * A DiffNode and its previous node mark a delta between two input sequences, that is, two differing subsequences
    ...
    extends PathNode
{
   ...
   * <p>
   * DiffNodes are compressed. That means that the path pointed to by the <code>prev</code> parameter will be followed
   * using {@link PathNode#previousSnake} until a non-diff node is found.
   *
   * @param the
      ...
      PathNode prev)
  {
    super(i,
          j,
          (prev == null ? null : prev.previousSnake()));
  }
