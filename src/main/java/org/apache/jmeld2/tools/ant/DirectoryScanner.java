 ...
 *
 */
package org.apache.jmeld2.tools.ant;
...
import org.apache.jmeld2.tools.ant.taskdefs.condition.Os;
import org.apache.jmeld2.tools.ant.types.Resource;
import org.apache.jmeld2.tools.ant.types.ResourceFactory;
import org.apache.jmeld2.tools.ant.types.selectors.FileSelector;
import org.apache.jmeld2.tools.ant.types.selectors.SelectorScanner;
import org.apache.jmeld2.tools.ant.types.selectors.SelectorUtils;
import org.apache.jmeld2.tools.ant.util.FileUtils;
    ...
    implements SelectorScanner, ResourceFactory
{
  ...
  /**
   * Set of all include patterns that are full file names and don't contain any
   * wildcards.
   *
   * <p>
   * If this instance is not case sensitive, the file names get turned to lower
   * case.
   * </p>
   *
   * <p>
  ...
  /**
   * Add to the list of exclude patterns to use. All '/' and '\' characters are
   * replaced by <code>File.separatorChar</code>, so the separator used need not
   * match <code>File.separatorChar</code>.
   * <p>
   * When a pattern ends with a '/' or '\', "**" is appended.
   *
   * @param excludes A list of exclude patterns. May be <code>null</code>, in
   *                 which case the exclude patterns don't get changed at all.
   *
   * @since Ant 1.6.3
   */
  ...
  /**
   * Scan the base directory for files which match at least one include pattern
    ...
    else
    {
      ...
      while (enum2.hasMoreElements())
      {
        ...
        if (myfile != null && myfile.exists())
        {
          ...
          {
            boolean included = isCaseSensitive() ? originalpattern.equals(currentelement)
  ...
  protected void scandir(File dir, String vpath, boolean fast)
  {
    ...
    {
      /*
       * two reasons are mentioned in the API docs for File.list (1) dir is not a
       * directory. This is impossible as we wouldn't get here in this case. (2) an IO
       * error occurred (why doesn't it throw an exception then???)
       */
  ...
  {
    if (filesIncludedMap.get(name) != null || filesExcluded.contains(name) || filesDeselected.contains(name))
    {
      return;
    }
  ...
  protected boolean couldHoldIncluded(String name)
  {
    ...
    for (String include : includes)
    {
      if (matchPatternStart(include, name, isCaseSensitive()) && isMorePowerfulThanExcludes(name, include)
  ...
  private boolean contentsExcluded(String name)
  {
    ...
    {
      if (exclude.endsWith("**")
          && SelectorUtils.matchPath(exclude.substring(0, exclude.length() - 2), name, isCaseSensitive()))
      {
        return true;
      }
    }
  ...
  /**
   * Return a cached result of list performed on file, if available. Invokes the
   * method and caches the result otherwise.
   *
  ...
  /**
   * From <code>base</code> traverse the filesystem in order to find a file that
  ...
  /**
   * From <code>base</code> traverse the filesystem in order to find a file that
   * matches the given stack of names.
   *
   * @param base         base File (dir).
  ...
  private boolean isSymlink(File base, List pathElements)
  {
    if (pathElements.size() > 0)
    {
      ...
      try
      {
        return FILE_UTILS.isSymbolicLink(base, current) || isSymlink(new File(base, current), pathElements);
      }
      catch (IOException ioe)
      {
        String msg = "IOException caught while checking " + "for links, couldn't get canonical path!";
