 ...
 *
 */
package org.apache.jmeld2.tools.ant.util;
...
import org.apache.jmeld2.tools.ant.BuildException;
...
public class FileUtils
{
  ...
  static
  {
    ...
    {
      isSpecial[i] = true;
      escapedChar1[i] = Character.forDigit(i >> 4,
                                           16);
      escapedChar2[i] = Character.forDigit(i & 0xf,
                                           16);
    }
    isSpecial[0x7f] = true;
    escapedChar1[0x7f] = '7';
    ...
    for (int i = 0; i < len; i++)
    {
      ch = escChs[i];
      isSpecial[ch] = true;
      escapedChar1[ch] = Character.forDigit(ch >> 4,
                                            16);
      escapedChar2[ch] = Character.forDigit(ch & 0xf,
                                            16);
    }
  }
   ...
   *
   * @param file
   *          the "reference" file for relative paths. This instance must be an absolute file and must not contain
   *          &quot;./&quot; or &quot;../&quot; sequences (same for \ instead of /). If it is null, this call is
   *          equivalent to <code>new java.io.File(filename)</code>.
   *
   * @param filename
   *          a file name.
   *
   * @return an absolute file that doesn't contain &quot;./&quot; or &quot;../&quot; sequences and uses the correct
   ...
   * </ul>
   * Unlike <code>File#getCanonicalPath()</code> this method specifically does not resolve symbolic links.
   *
   * @param path
  ...
  public File normalize(String path)
  {
    ...
    if ((onDos && path.length() >= 2 && Character.isLetter(path.charAt(0)) && path.charAt(1) == ':')
        || (onNetWare && colon > -1))
    {
      dosWithDrive = true;

      char[] ca = path.replace('/',
                               '\\')
          .toCharArray();
  ...
  public String toVMSPath(File f)
  {
    ...
    boolean isDirectory = f.isDirectory() && !name.regionMatches(true,
                                                                 name.length() - 4,
                                                                 ".DIR",
                                                                 0,
                                                                 4);
   ...
   * </p>
   * <p>
   * This method is different from File.createTempFile() of JDK 1.2 as it doesn't create the file itself. It uses the
   * location pointed to by java.io.tmpdir when the parentDir attribute is null.
   * </p>
   *
   * @param prefix
      ...
      throws IOException
  {
    ...
    if (fileNameEquals(f1,
                       f2))
    {
      // same filename => true
      return true;
    }
    return textfile ? textEquals(f1,
                                 f2)
                    : binaryEquals(f1,
                                   f2);
  }
   ...
   * <p>
   * simple but sub-optimal comparision algorithm. written for working rather than fast. Better would be a block read
   * into buffers followed by long comparisions apart from the final 1-7 bytes.
   * </p>
   *
   * @param f1
   *          the file whose content is to be compared.
   ...
   *           if the files cannot be read.
   */
  private boolean binaryEquals(File f1,
      File f2)
      throws IOException
  {
   ...
   *           if the files cannot be read.
   */
  private boolean textEquals(File f1,
      File f2)
      throws IOException
  {
      ...
      throws IOException
  {
    ...
    while (bufferLength != -1)
    {
      ...
      if (bufferLength > 0)
      {
        ...
        textBuffer.append(new String(buffer,
                                     0,
                                     bufferLength));
      }
    }
      ...
      throws IOException
  {
    ...
    File toTest = new File(parent.getCanonicalPath(),
                           name);
  ...
  public boolean fileNameEquals(File f1,
      File f2)
  {
    return normalize(f1.getAbsolutePath()).equals(normalize(f2.getAbsolutePath()));
  }

  /**
   * Get the granularity of file timestamps. The choice is made based on OS, which is incorrect--it should really be by
   * filesystem. We do not have an easy way to probe for file systems, however.
   * 
   * @return the difference, in milliseconds, which two file timestamps must have in order for the two files to be given
   *         a creation order.
   */
  public long getFileTimestampGranularity()
  {
    return onDos ? FAT_FILE_TIMESTAMP_GRANULARITY : UNIX_FILE_TIMESTAMP_GRANULARITY;
  }

  /**
   * Returns true if the source is older than the dest. If the dest file does not exist, then the test returns false; it
   * is implicitly not up do date.
   * 
   * @param source
