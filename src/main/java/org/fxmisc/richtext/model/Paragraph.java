...
/**
 * One paragraph in the document that can itself be styled and which contains a
 * list of styled segments.
 *
 * <p>
 * It corresponds to a single line when the text is not wrapped or spans
 * multiple lines when the text is wrapped. A Paragraph contains of a list of
 * {@link SEG} objects which make up the individual segments of the Paragraph.
 * By providing a specific segment object and an associated {@link SegmentOps
 * segment operations} object, all required data and the necessary operations on
 * this data for a single segment can be provided.
 * </p>
 *
 * <p>
 * For more complex requirements (for example, when both text and images shall
 * be part of the document), a different segment type must be provided. One
 * should use something like {@code Either<String, Image>} for their segment
 * type.
 *
 * <b>Note that Paragraph is an immutable class</b> - to modify a Paragraph, a
 * new Paragraph object must be created. Paragraph itself contains some methods
 * which take care of this, such as concat(), which appends some Paragraph to
 * the current one and returns a new Paragraph.
 * </p>
 *
 * @param <PS>  The type of the paragraph style.
 * @param <SEG> The type of the content segments in the paragraph (e.g.
 *              {@link String}). Every paragraph, even an empty paragraph, must
 *              have at least one {@link SEG} object (even if that {@link SEG}
 *              object itself represents an empty segment).
...
public final class Paragraph<PS, SEG, S>
{
  ...
  /**
   * Creates a paragraph that has only one segment that has the same given style
   * throughout.
   */
  ...
  /**
   * Creates a paragraph that has only one segment but a number of different
   * styles throughout that segment
   */
  ...
  /**
   * Creates a paragraph that has multiple segments with multiple styles
  ...
  /**
   * Since the segments and styles in a paragraph are stored separate from
   * another, combines these two collections into a single collection where each
   * segment and its corresponding style are grouped into the same object.
  ...
  public List<StyledSegment<SEG, S>> getStyledSegments()
  {
    if (styledSegments == null)
    {
      ...
      {
        styledSegments = Collections
            .singletonList(new StyledSegment<>(segments.get(0), styles.getStyleSpan(0).getStyle()));
      }
      else
      {
  ...
  /**
   * Concatenates this paragraph with the given paragraph {@code p}. The paragraph
   * style of the result will be that of this paragraph, unless this paragraph is
   * empty and {@code p} is non-empty, in which case the paragraph style of the
   * result will be that of {@code p}.
   */
  public Paragraph<PS, SEG, S> concat(Paragraph<PS, SEG, S> p)
  {
    ...
    if (merge.isPresent())
    {
      ...
      updatedStyles = updatedLeftSpan.append(merge.get(), leftSpan.getLength() + rightSpan.getLength())
  ...
  /**
   * Similar to {@link #concat(Paragraph)}, except in case both paragraphs are
   * empty, the result's paragraph style will be that of the argument.
   */
  ...
  public Paragraph<PS, SEG, S> subSequence(int start)
  {
    ...
    {
      // in case one is using EitherOps<SegmentOps, SegmentOps>, force the empty
      // segment
   ...
   * @return A new paragraph with the same segment contents, but a new paragraph
   *         style.
   */
  ...
  /**
   * Returns the style of character with the given index. If {@code charIdx < 0},
   * returns the style at the beginning of this paragraph. If
   * {@code charIdx >= this.length()}, returns the style at the end of this
   * paragraph.
   */
  ...
  /**
   * Returns the style at the given position. That is the style of the character
   * immediately preceding {@code position}. If {@code position} is 0, then the
   * style of the first character (index 0) in this paragraph is returned. If this
   * paragraph is empty, then some style previously used in this paragraph is
   * returned. If {@code position > this.length()}, then it is equivalent to
   ...
   * {@code position} points to a boundary between two styled ranges, then the
   * range preceding {@code position} is returned.
   */
  ...
  /**
   * Returns the plain text content of this paragraph, not including the line
   * terminator.
   */
  public String getText()
  {
    if (text == null)
    {
      ...
      for (SEG seg : segments) sb.append(segmentOps.getText(seg));
      text = sb.toString();
    }
    return text;
  }
  ...
  public String toString()
  {
    return "Par[" + paragraphStyle + "; "
        + getStyledSegments().stream().map(Object::toString).reduce((s1, s2) -> s1 + ", " + s2).orElse("") + "]";
  }

  /**
   * Two paragraphs are defined to be equal if they have the same style (as
   * defined by PS.equals) and the same list of segments (as defined by
   * SEG.equals).
   */
  @Override
  ...
  public void replaceParagraphStyle(PS paragraphStyle)
  {
    Paragraph.this.paragraphStyle = paragraphStyle;
  }

  public void replaceStyle(int from, int to, S style)
  {
    StyleSpans<S> left;
    StyleSpans<S> right;
    StyleSpans<S> newStyles;

    left = styles.subView(0, from);
    right = styles.subView(to, length());
    newStyles = left.append((S) Collections.singletonList(style), to - from).concat(right);
