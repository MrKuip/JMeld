...
import org.eclipse.compare2.internal.core.LCS;
    ...
    extends LCS
{
  ...
  {
    RangeComparatorLCS lcs = new RangeComparatorLCS(left,
                                                    right);
    try
    {
      lcs.longestCommonSubsequence();
      return lcs.getDifferences(factory);
    }
    finally
    {
    }
  ...
  @Override
  protected boolean isRangeEqual(int i1,
      int i2)
  {
    return this.comparator1.rangesEqual(i1,
                                        this.comparator2,
                                        i2);
  }

  @Override
  protected void setLcs(int sl1,
      int sl2)
  {
  ...
  public RangeDifference[] getDifferences(AbstractRangeDifferenceFactory factory)
  {
    try
    {
      ...
      if (length == 0)
      {
        differences.add(factory.createRangeDifference(RangeDifference.CHANGE,
                                                      0,
                                                      this.comparator2.getRangeCount(),
                                                      0,
                                                      this.comparator1.getRangeCount()));
      }
      else
      {
        int index1, index2;
        ...
        if (s1 != -1 && (s1 + 1 < this.comparator1.getRangeCount() || s2 + 1 < this.comparator2.getRangeCount()))
        {
          ...
          differences.add(factory.createRangeDifference(RangeDifference.CHANGE,
                                                        rightStart,
                                                        this.comparator2.getRangeCount() - (s2 + 1),
                                                        leftStart,
                                                        this.comparator1.getRangeCount() - (s1 + 1)));
        }
  ...
  /**
   * This method takes an LCS result interspersed with zeros (i.e. empty slots from the LCS algorithm), compacts it and
   * shifts the LCS chunks as far towards the front as possible. This tends to produce good results most of the time.
   *
   * @param lcsSide
   *          A subsequence of original, presumably it is the LCS of it and some other collection of lines
   * @param length
   *          The number of non-empty (i.e non-zero) entries in LCS
   * @param comparator
   *          The comparator used to generate the LCS
   */
  private void compactAndShiftLCS(int[] lcsSide,
      int length,
      IRangeComparator comparator)
  {
    ...
    for (int i = 1; i < length; i++)
    {
      ...
      if (nextLine != lcsSide[j] && comparator.rangesEqual(nextLine - 1,
                                                           comparator,
                                                           lcsSide[j] - 1))
      {
        lcsSide[i] = nextLine;
  ...
  public void longestCommonSubsequence()
  {
    ...
    { // The LCS can be null if one of the sides is empty
      compactAndShiftLCS(this.lcs[0],
                         getLength(),
                         this.comparator1);
      compactAndShiftLCS(this.lcs[1],
                         getLength(),
                         this.comparator2);
    }
  }
