 ...
 * details):
 * <ul>
 * <li>Time-consuming tasks should not be run on the <i>Event Dispatch Thread</i>. Otherwise the application becomes
 * unresponsive.</li>
 ...
 * activities should happen. To notify {@code PropertyChangeListeners} about bound properties changes use the
 * {@link #firePropertyChange firePropertyChange} and {@link #getPropertyChangeSupport} methods. By default there are
 * two bound properties available: {@code state} and {@code progress}.
 * <li>
 * <p>
 ...
 * computing, we want to update our progress in a {@code JProgressBar}. Finally, we also want to print the prime numbers
 * to {@code System.out}. <pre>
 ...
 * @param <V>
 *          the type used for carrying out intermediate results by this {@code SwingWorker's} {@code publish} and
    ...
    implements Future<T>, Runnable
{
  ...
  /**
   * Sends data chunks to the {@link #process} method. This method is to be used from inside the {@code doInBackground}
   * method to deliver intermediate results for processing on the <i>Event Dispatch Thread</i> inside the
   * {@code process} method.
   * 
   * <p>
   ...
   *
   * <p>
   * <b>Sample Usage</b>. This code snippet loads some tabular data and updates {@code DefaultTableModel} with it. Note
   * that it safe to mutate the tableModel from inside the {@code process} method because it is invoked on the <i>Event
   * Dispatch Thread</i>.
   * 
   * <pre>
   ...
   * For performance purposes all these invocations are coalesced into one invocation with the last invocation argument
   * only.
   * 
   * <p>
  ...
  protected final void setProgress(int progress)
  {
    ...
    doNotifyProgressChange.add(oldProgress,
                               progress);
  }
   ...
   *
   * <p>
   * Note: {@code SwingWorker} is only designed to be executed once. Executing a {@code SwingWorker} more than once will
   * not result in invoking the {@code doInBackground} method twice.
   */
  ...
  /**
   * Adds a {@code PropertyChangeListener} to the listener list. The listener is registered for all properties. The same
   * listener object may be added more than once, and will be called as many times as it is added. If {@code listener}
   * is {@code null}, no exception is thrown and no action is taken.
   * 
   * <p>
   * Note: This is merely a convenience wrapper. All work is delegated to {@code PropertyChangeSupport} from
   * {@link #getPropertyChangeSupport}.
   * 
   * @param listener
  ...
  /**
   * Removes a {@code PropertyChangeListener} from the listener list. This removes a {@code PropertyChangeListener} that
   * was registered for all properties. If {@code listener} was added more than once to the same event source, it will
   * be notified one less time after being removed. If {@code listener} is {@code null}, or was never added, no
   ...
   *
   * <p>
   * When called off the <i>Event Dispatch Thread</i> {@code PropertyChangeListeners} are notified asynchronously on the
  ...
  /**
   * Returns the {@code PropertyChangeSupport} for this {@code SwingWorker}. This method is used when flexible access to
   * bound properties support is needed.
   * <p>
   * This {@code SwingWorker} will be the source for any generated events.
   * 
   * <p>
   * Note: The returned {@code PropertyChangeSupport} notifies any {@code PropertyChangeListener}s asynchronously on the
   * <i>Event Dispatch Thread</i> in the event that {@code firePropertyChange} or {@code fireIndexedPropertyChange} are
   * called off the <i>Event Dispatch Thread</i>.
   * 
